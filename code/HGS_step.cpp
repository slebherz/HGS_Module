...
{   
	...
	// The absolute period is used to identify the files written by HGS for
	// the current timestep -- files are written with a .[timestep#] extension.
	int period = (int)argument[0];
...
void WriteOasisOutput(float argument[])
{
	...
	double delivery350 = argument[1];
	double delivery650 = argument[2];
	double delivery651 = argument[3];
	double delivery652 = argument[4];
	double delivery850 = argument[5];
	double delivery950 = argument[6];
	double delivery951 = argument[7];
	// Releases (in cms) from all reservoirs in the model.
	double flow295_290 = argument[8];
	double flow395_390 = argument[9];
	double flow995_980 = argument[10];
	// Boolean that identifies whether MDS is met for the timestep.
	bool MDS_IsMet = argument[11];
	...
	// Write the MDS flag.
	fOasisMDS << std::fixed << MDS_IsMet << "\t\t!1: MDS satisfiied, 0: MDS not satisfied" << std::endl;
	fOasisMDS.close();
}
...
void WaitForHGS(int period, std::string fnameHGS)
{
	...
	//	c. HGS writes output for the current timestep to the next
	//	   timestep) until HGS has read the data for the current timestep.
...
void ReadHGSInput(float argument[], std::string fnameHGS)
{
	...
	// Parse the HGS output on timesteps except for the very first. The models
	// run independently for the first timestep.
	if(period > 1)
	{
